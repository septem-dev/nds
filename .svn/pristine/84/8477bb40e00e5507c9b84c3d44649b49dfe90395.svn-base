...
document.querySelectorAll('.tab-list').forEach(tabList => {
  tabList.querySelectorAll('.tab__item').forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      tabList.querySelectorAll('.tab__item').forEach(t => t.classList.remove('active'));
      tab.classList.add('active');

      // data-tab 속성이 있으면 패널 전환
...
const TOAST_ICONS = {
  ...
  default : `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="flex-shrink:0;margin-top:2px"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>`,
};

function showToast({ title, message, type = '', duration = 3000 }) {
  const container = getToastContainer();
  const toast = document.createElement('div');
  toast.className = `toast${type ? ' toast--' + type : ''} toast--anim`;
...
document.querySelectorAll('[data-toast]').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    const cfg  = TOAST_PRESETS[type] || TOAST_PRESETS[''];
    showToast({ ...cfg, type });
  });
});
...
const navLinks  = document.querySelectorAll('.g-sidebar nav a');
const navObserver = new IntersectionObserver(entries => {
  entries.forEach(e => {
    if (e.isIntersecting) {
      navLinks.forEach(l => l.classList.remove('active'));
      const link = document.querySelector(`.g-sidebar nav a[href="#${e.target.id}"]`);
